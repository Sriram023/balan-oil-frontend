...
import { useCustomers } from "@/context/CustomerContext"; // 👈 make sure the path matches your file structure
import { fetchCustomerById, addCredit, addPayment, Customer, Transaction, } from "@/api";
/* ---------- Types ---------- */
interface Product {
  id: number;
  name: string;
...
const buildProductsHTML = (
  ...
  return `
    <html>
      <head>
        <title>${customerName} - Products Purchased</title>
        <meta charset="utf-8" />
        <style>
      ...
      <body>
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
          <div>
            ...
            <div class="muted">Products Purchased from ${customerName}</div>
          </div>
...
const CustomerDetail = () => {
  const { id } = useParams();
const { customers } = useCustomers(); // ✅ must come first
console.log("🧭 URL ID:", id);
console.log("🗂️ Customers in context:", customers);
    ...
    return (
      <DashboardLayout>
        <div className="p-6">
          ...
          <p className="mt-4 text-red-600">Customer not found 😕</p>
        </div>
      </DashboardLayout>
    );
  }
  
  const balance = customer.creditGiven - customer.paidAmount;
...
const handleAddProduct = async (e: React.FormEvent) => {
  ...
  try {
    ...
    const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/customers/${customer._id || customer.id}/add-product`, {
      method: "POST",
  ...
  const handlePrintProducts = () => {
    ...
    const html = buildProductsHTML(customer.name, products, totals);
